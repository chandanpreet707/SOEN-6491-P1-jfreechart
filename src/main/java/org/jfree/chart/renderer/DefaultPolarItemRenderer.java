...
public class DefaultPolarItemRenderer extends AbstractRenderer
     ...
     * will be drawn. If not active, NaN values will be drawn on top-left of
     * the plot.
     */
    ...
    /**
     * Returns {@code true} if NaN values should be hidden and a gap in the
     * line will be drawn. If not active, NaN values will be drawn in the
    ...
    /**
     * Sets the flag that controls whether NaN values should be hidden and a
     * gap in the line will be drawn. If not active, NaN values will be drawn
    ...
    public void drawSeries(Graphics2D g2, Rectangle2D dataArea,
        ...
        for (int i = 0; i < numPoints; i++) {
            ...
            // Skip NaN values and start a new segment after the gap
            if (getHideNaNValues()
                    && (Double.isNaN(theta) || Double.isNaN(radius))) {
        ...
        // Handle connecting first and last point
        if (getConnectFirstAndLastPoint()) {
            if (getHideNaNValues()) {
                ...
                // Only connect if both first and last points are not NaN
                if (!Double.isNaN(dataset.getXValue(seriesIndex, numPoints - 1))
                        && !Double.isNaN(dataset.getYValue(seriesIndex, numPoints - 1))
                        && !Double.isNaN(thetaStart)
                        && !Double.isNaN(radiusStart)) {

                    Point p = plot.translateToJava2D(thetaStart, radiusStart,
                            axis, dataArea);
                    poly.lineTo(p.getX(), p.getY());
                }
            } else {
